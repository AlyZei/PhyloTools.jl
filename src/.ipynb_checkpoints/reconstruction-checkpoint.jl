...
Base.@kwdef mutable struct ProbabilityOnTreeRNA <: TreeNodeData # Create a custom data type
    prob :: Array{Float64,1} = zeros(Float64,4)
end
...
function FelsensteinSingle2rna!(logZn::Float64, TransitionProb::Array{Float64,2}, T, n::String, m::Float64, p, q::Int)
    # Initialize LogNodeProb without in-place modification later
    LogNodeProb = zeros(Float64, q)
    ...
    for child in children(T[n])
        if isleaf(child)
            ...
            # Recursive call
            ChildProb, logZn = FelsensteinSingle2rna!(logZn, TransitionProb, T, label(child), m, p, q)
        end
    ...
    # Update tree data
    data!(T[n], ProbabilityOnTreeRNA(prob = NodeProb))
...
function Felsenstein2rna(Z::AbstractDict, S::Array{Float64,2}, T, n::String, m::Float64; q::Int = 21, verbose::Bool = false, eps::Float64 = 10^-5)
    ...
    @tasks for s in 1:L
        ...
        for leaf in leaves(_T)
            ρ = zeros(Float64,q)
            ρ[Z[label(leaf)][s]] = 1
            data!(leaf, ProbabilityOnTreeRNA(prob = ρ))
        end
        #AncestorProb[s,:] = FelsensteinSingle2!(TransitionProb, _T, n, m, view(S,s,:), q)
        logZn = 0.
        a,b = FelsensteinSingle2rna!(logZn, TransitionProb, _T, n, m, view(S,s,:), q)
        AncestorProb[s,:] = a .* S[s,:]
        normal = sum(AncestorProb[s,:])
        AncestorProb[s,:] ./= normal
        Ancestor[s] = argmax(view(AncestorProb,s,:))
...
function Felsenstein2(FileNat::String, FileTree::String, FileSequences::String, m::Float64; verbose::Bool = false, eps::Float64 = 10^-5, infer_w::Bool = false)
  
    _, NatMSA, _, _, q = read_fasta(FileNat,1.0,0.2,false)
    if infer_w == false
        StationaryProb = compute_empirical_freqs(Int.(NatMSA), q; eps = eps)
    else
        StationaryProb = Float64.(run_inference(FileSequences, FileTree, FileSequences, m)')
    end 
...
function Felsenstein2rna(FileNat::String, FileTree::String, FileSequences::String, m::Float64; verbose::Bool = false, eps::Float64 = 10^-5, infer_w::Bool = false)
  
    NatMSA = read_rna(FileSequences,0.9);
    q = 4;
    if infer_w == false
        StationaryProb = compute_empirical_freqs(Int.(NatMSA), q; eps = eps)
    else
    ...
    LeavesSequences = read_fasta_dict_rna(FileSequences)
    MyTree = read_tree(FileTree, node_data_type = ProbabilityOnTreeRNA)

    AncestorSequence, AncestorProbability, AncLike = Felsenstein2rna(LeavesSequences, StationaryProb, MyTree, label(root(MyTree)), m; q = q, verbose = verbose, eps=eps)
