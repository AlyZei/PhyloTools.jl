...
"""
function ham_dist(vec1::Array{T,1}, vec2::Array{T,1}) where {T}
...
"""
function ham_dist(vec::Array{T,1}, msa::Array{T,2}) where {T}
...
"""
function ham_dist(msa1::Array{T,2}, msa2::Array{T,2}) where {T}
...
"""
function ham_dist(step_msa::Array{Array{T,2},1}) where {T}
...
"""
function ham_dist_AB_rp2(step_msa::Array{T,3}, step_msa_B::Array{T,3}, num::Int) where {T}
...
"""
function ham_dist_AB_rp(step_msa::Array{Array{T,2},1}, step_msa_B::Array{Array{T,2},1}, num::Int) where {T}
    N_steps = length(step_msa)
...
"""
function pairwise_ham_dist(msa::Array{T,2}; n_seq = 100, all = false) where {T}
    res = []
    for i in 1:n_seq
        for j in i+1:n_seq
